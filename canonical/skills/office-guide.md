...
targets: [all]
triggers: [cohort office, office roster, specialist agents, chief of staff, which specialist]
---
...
- **Build loops.** `/build` is the plan-driven **inner** loop (implement–test–verify–commit).
  ...
  `/crew` is the **fan-out** loop for substantive work: a coordinator-tier session
  (Fable preferred, Opus a full coordinator too — never below Opus) researches and plans,
  routes tasks to model tiers (fable/opus/sonnet/haiku, max 20 agents in flight), and
...
- **Second opinions across vendors.** `/consult-gpt` brings ChatGPT into the office as an
  ...
  analysis, an untrusted claim to verify, never executed; `/crew` consults it on
