...
2. **`security-engineer`** — Run a vulnerability and threat-model pass. Check OWASP Top 10, secrets handling, auth/authz, dependency CVEs. Output the standard audit report.
...
2. **Security** — Promote any Critical/High `security-engineer` findings to launch blockers. Cross-reference with `code-reviewer`'s security axis.
