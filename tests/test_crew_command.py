"""Wording-lock for the compiled /crew command.

/crew is the standard fan-out development protocol, widened across vendors: the
coordinating session (Fable or Opus) researches, plans, routes tasks to model
tiers *and vendors*, and signs off. Claude subagents write directly; external
...
- the **isolation rule** — parallel writers need per-task worktrees and never
  commit in the coordinator's shared checkout;
- the **worker kickback** — a worker returns a beyond-tier task from below;
- the **tier routing** table (fable/opus/sonnet/haiku);
- the **coordinator floor** — never coordinate below Opus; and
- **Opus as a first-class coordinator** that escalates a genuinely Fable-suited
  task to the user.
...
def _compiled_crew_body() -> str:
    """Return the compiled Claude /crew command body (fresh from the renderer)."""
    staged = {sf.staged_rel: sf.content for sf in compile_ide(REPO, "claude").staged}
    rel = "commands/crew.md"
    assert rel in staged, f"/crew did not compile for claude; got {sorted(staged)}"
...
def test_crew_locks_parallel_writer_isolation():
    body = _compiled_crew_body()
    # Concurrent writers must use per-task worktrees, and workers may not commit
    # in the coordinator's shared checkout.
...
def test_crew_locks_the_worker_kickback():
    ...
    flat = " ".join(_compiled_crew_body().split())
...
def test_crew_routes_all_four_tiers():
    body = _compiled_crew_body()
    for tier in ("fable", "opus", "sonnet", "haiku"):
        assert f"**{tier}**" in body, f"tier {tier} missing from routing table"


def test_crew_keeps_coordination_on_the_top_level_session():
    body = _compiled_crew_body()
    assert "never delegates the plan or the signoff to a subagent" in body


def test_crew_holds_the_claude_writes_external_proposes_line():
    # The cross-vendor invariant: Claude subagents write directly; external
...
def test_crew_opus_is_a_first_class_coordinator():
    # A native Opus session coordinates in its own right (not just a Fable
...
def test_crew_opus_escalates_a_fable_suited_task_to_the_user():
    ...
    flat = " ".join(_compiled_crew_body().split())
    assert "genuinely better suited to Fable" in flat
...
def test_crew_never_coordinates_below_opus():
    # The pattern must never repeat on Sonnet/Haiku: recommend switching up.
    body = _compiled_crew_body()
    flat = " ".join(_compiled_crew_body().split())
    assert "do not run `/crew`" in body
