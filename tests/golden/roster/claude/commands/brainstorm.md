...
**Rotate the tier across runs**, as `/audit` does: read the last phase from the ledger and
advance `balanced` → `complex-heavy` → `simple-heavy` → `balanced`, recording the phase and
the tier per lens. Divergence is where this matters most — tiers do not just differ in
*quality*, they differ in *what they think of*, and a lens always generated by one tier keeps
returning that tier's ideas. A complex-heavy divergence run is not wasteful here; it is a
different idea distribution. The judgment steps keep their floor regardless of phase:
converge, red-team and pursue never drop below Fable/Opus.

**Confirm each vendor is actually reachable before routing to it**, and say in the ledger
which vendors answered. A "cross-vendor" idea sweep that silently ran on one vendor is the
same failure `/audit` names under vendor-reachability, and it is harder to notice here —
ideas look fine on their own.
